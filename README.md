...
There are three components of the user's guide that have separate mechanisms of translation: text in the source RST files; text provided by Sphinx such as "Note" or "Warning"; and text generated by the custom `:investspec:` role that's imported from `natcap.invest`.
