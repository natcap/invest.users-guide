...
where :math:`y` indexes all grid cells on :math:`r` unicode:: U+2018 s raster map and :math:`Y_r` indicates the set of grid cells on :math:`r` unicode:: U+2019 s raster map.  Note that each threat map can have a unique number of grid cells due to variation in raster resolution   If :math:`S_{jr} = 0` then :math:`D_{xj}` is not a function of threat :math:`r`.  Also note that threat weights are normalized so that the sum across all threats weights equals 1.
