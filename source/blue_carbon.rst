...
If data is lacking, the model can be used with any subset of the four carbon pools. To determine the tonnes of CO_2 equivalent stored across the whole seascape at time t, we sum all grid cell carbon storage values.
...
We estimate accumulation by multiplying habitat specific rates of carbon accumulation by the total area of habitat. The carbon sequestered in a grid cell `x` at time `t`, given by `\Delta C_{xt}` and measured in tons of CO_2 equivalent per year, is equal to the rate of carbon accumulation in the sediments at time `t`, where `A_{xjt}` is the area of vegetation `j` in grid cell `x` at time `t`.
...
+--------------------------------+----------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------+--------------------------------------+
| Methane emissions              | 1.85 T CO_2e/ha/yr (4)                                                                              | 0.4 t CO_2/ha/yr                                                                                                                                    | negligible                                                                    | Use literature / field data          |
+--------------------------------+----------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------+--------------------------------------+
 ...
 * **CSV or DBF file**: containing information on carbon storage in biomass (tons of CO_2e/ha), sediments (tons of CO_2e/ha), rate of accumulation (tons of CO_2e/ha/yr) and depth (in meters) of sediments for each type of marine vegetation.  
 ...
 * **value of a sequestered ton of carbon**: (V in the equation below), in dollars per metric ton of elemental carbon (not CO_2, which is heavier, so be careful to get units right! If the social value of CO_2e is $Y per metric ton, then the social value of C is $(3.67*Y) per metric ton (Labeled “Price of carbon per metric ton (optional)” in the tool interface.) For applications interested in estimating the total value of carbon sequestration, we recommend value estimates based of damage costs associated with the release of an additional ton of carbon (the social cost of carbon (SCC).
