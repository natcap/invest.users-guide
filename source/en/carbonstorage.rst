...
* **[Workspace]** folder:
	...
	* **report_[Suffix].html:** This file presents a summary of all data computed by the model. It also includes descriptions of all other output files produced by the model, so it is a good place to begin exploring and understanding model results. Because this is an HTML file, it can be opened with any web browser.
		...
		* The information in the **Aggregate Results** box represents total values, summarized across the whole area of interest. So, **Storage - Baseline** is the total amount of carbon within the area of interest for the baseline LULC map, summarized from the raster result c_storage_cur.tif; **Storage - Alternate** is the total amount of carbon within the area of interest for the alternate LULC map, summarized from the raster result c_storage_fut.tif; and **Change in C for Alternate** is the total amount of change between baseline and alternate within the area of interest.
