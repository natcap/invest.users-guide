...
* **[Workspace]** folder:
	...
	* **report_[Suffix].html:** This file presents a summary of all data computed by the model. It also includes descriptions of all other output files produced by the model, so it is a good place to begin exploring and understanding model results. Because this is an HTML file, it can be opened with any web browser.
		...
		* The information in the **Aggregate Results** box represents total values, summarized across the whole area of interest. So, **Total cur** is the total amount of carbon within the area of interest for the current LULC map; **Total fut** is the total amount of carbon within the area of interest for the future LULC map; and **Change in C for fut** is the total amount of change between current and future within the area of interest. 
