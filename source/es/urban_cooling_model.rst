...
* **hm_[Sufijo].tif**: The calculated HMI.
* **cc_[Sufijo].tif**: Ráster de valores CE.
* **uhi_results_[Sufijo].shp**: Una copia del vector de input "Área de interés" con los siguientes campos adicionales:
    * **avg_cc**: Valor medio de CE (unitless).
...
La carpeta ``intermediate`` contiene resultados adicionales del modelo:
...
* **eti_[Sufijo].tif**: Raster of evapotranspiration index (reference evapotranspiration (ETo) times crop coefficient (Kc) divided by ETmax).
...
* **T_air_nomix_[Sufijo].tif**: Ráster de los valores de temperatura del aire estimados antes de la mezcla del aire (es decir, antes de aplicar el algoritmo de media móvil).
