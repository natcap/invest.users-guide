...
Ensuring the persistence of populations, species and communities requires maintaining habitat of sufficient extent and quality to support their existence. InVEST models habitat quality and rarity, ultimately estimating the extent of habitat across a landscape and its state of degradation. In this model, habitat quality is a function of four factors: each threat’s relative impact, the relative sensitivity of each habitat type to each threat, the distance between habitats and sources of threats, and the degree to which the land is protected from threats. Required inputs include a LULC map, the sensitivity of LULC types to each threat, spatial data on the distribution and intensity of each threat and the location of protected areas. The habitat quality model assumes that the protection of land is effective and that all threats to a landscape are additive. Habitat rarity is a function of the current extent of a habitat type, relative to its baseline extent.

Introduction
============

A primary goal of conservation is the protection of biodiversity, which includes the variety of genes, species, populations, communities and ecosystems. Maintaining habitat that supports species and communities of interest can contribute to safeguarding important aspects of biodiversity. Here we treat habitat quality as an attribute of ecosystems and not an ecosystem service per se. InVEST does not provide a direct means of assigning monetary value to habitat quality. Instead, the habitat quality and rarity model is best used to evaluate synergies and trade-offs between habitat conservation, ecosystem service provision and other management goals. Natural resource managers, corporations and conservation organizations are becoming increasingly interested in understanding how and where habitat and ecosystem service provision align in space and how management actions affect both.
...
Note that habitat quality as modeled in InVEST is best interpreted as an indicator of the potential extent and abundance of an individual species, or of a group of similar species. This captures only a limited part of the many dimensions of biodiversity (Noss 1990). While some species (such keystone species and umbrella species) may effectively serve as an indicator of other facets of biodiversity, this should not be assumed without empirical evidence (Noss 1990). Results from the InVEST habitat quality model should not by default be assumed to provide a measure of the overall biodiversity levels of a landscape.

The reasons for modeling habitat quality alongside ecosystem services are simple and powerful. Doing so allows us to compare spatial patterns between different conservation objectives, and to identify win-win areas (i.e., areas where conservation can benefit both natural systems and human economies) as well as areas where these goals are not aligned. Further, it allows us to analyze trade-offs between habitat quality and ecosystem services across differing scenarios of future land use change. Land use/land cover (LULC) patterns that generate greater ecosystem service production may not always lead to greater conservation of key species, communities or habitats (Nelson et al. 2008), and modeling future options today can help identify and avoid tradeoffs.

The Model
=========

The InVEST habitat quality model combines information on LULC and habitat threats to produce habitat quality maps. This approach generates two key sets of information that are useful in making an initial assessment of conservation needs: the relative extent and degradation of different types of habitat types in a region and changes across time. This approach further allows rapid assessment of the status of and change in a proxy for conservation status. If habitat changes are taken as representative of genetic, species, or community changes, the user is assuming that areas with high quality habitat will better support those particular aspects of biodiversity and that decreases in habitat extent and quality over time means a decline in the abundance, persistence or  resilience of those aspects under consideration in the area of decline.

The habitat rarity model indicates the extent and pattern of land cover types on the current or a potential future landscape vis-a-vis the extent of the same land cover types in some baseline period. Rarity maps allow users to create a map of the rarest habitats on the landscape relative to the baseline chosen by the user to represent the mix of habitats on the landscape that is most appropriate for the study area’s baseline conditions.

The model requires basic data that can be assembled for virtually any location in the world, making it useful in areas for which species distribution data are poor or lacking altogether. Extensive occurrence (presence/absence) data may be available in many places for current conditions, and can be used to calibrate or validate model results. However, modeling the change in occurrence, persistence, or vulnerability of multiple species under future conditions is often impossible or infeasible. While a habitat approach leaves out the detailed species occurrence data available for current conditions, several of its components represent advances in functionality over many existing conservation planning tools. The most significant is the ability to characterize the sensitivity of habitats types to various threats. Not all habitats are affected by all threats in the same way, and the InVEST model accounts for this variability. Further, the model allows users to estimate the relative impact of one threat over another so that threats that are more damaging to habitat quality on the landscape can be represented as such. For example, grassland could be particularly sensitive to threats generated by urban areas yet moderately sensitive to threats generated by roads. In addition, the distance over which a threat will degrade natural systems can be incorporated into the model.
...
1. The first factor is the relative impact of each threat. Some threats may be more damaging to habitat, all else equal, and a relative impact score accounts for this (see Table 1 for a list of possible threats).  For instance, urban areas might be considered to be twice as degrading to any nearby habitats as agricultural areas. A degradation source's weight, :math:`w_r`, indicates the relative destructiveness of a degradation source to all habitats.  The weight :math:`w_r` can take on any value from 0 to 1.  For example, if urban area has a threat weight of 1 and the threat weight of roads is set equal to 0.5 then the urban area causes twice the disturbance, all else equal, to all habitat types. To reiterate, if we have assigned species group-specific habitat suitability scores to each LULC then the threats and their weights should be specific to the modeled species group.
...
In the first step of the rarity calculation we take the ratio between the current or projected and past (baseline) extents of each LULC type :math:`j`. Subtracting this ratio from one, the model derives an index that represents the relative rarity of that LULC class on the landscape of interest.

.. math:: R_j=1-\frac{N_j}{N_{j_\mathrm{baseline}}}
   :label: eqn5


where :math:`N_j` is the number of grid cells of LULC :math:`j` on the current or projected map and :math:`N_{j_\mathrm{baseline}}` gives the number of grid cells of LULC :math:`j` on the baseline landscape.  The calculation of :math:`R_j` requires that the baseline, current, and/or projected LULC maps are all in the same resolution.  In this scoring system, the closer to 1 a LULC's :math:`R` score is, the greater proportional losses have occurred for that LULC type. If LULC j did not appear on the baseline landscape then we set :math:`R_j = 0`.
...
Modifying Output and Creating a Landscape Habitat Quality Score
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
