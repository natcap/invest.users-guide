...
The model is available as a standalone application accessible from the Windows start menu.  For Windows 7 or earlier, this can be found under *All Programs -> InVEST +VERSION+ -> Hydropower Water Yield*.  Windows 8 users can find the application by pressing the windows start key and typing "water yield" to refine the list of applications.  The standalone can also be found directly in the InVEST install directory under the subdirectory *invest-3_x86/hydropower_water_yield.exe*.
