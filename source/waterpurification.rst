...
where ALVx is the Adjusted Loading Value at pixel x, polx is the export coefficient at pixel x, and HSSx  is the Hydrologic Sensitivity Score at pixel x which is calculated as:  

					      

where  is the runoff index at pixel x, calculated using the following equation, and  is the mean runoff index in the watershed of interest.  

					 

where  is the sum of the water yield of pixels along the flow path above pixel x (it also includes the water yield of pixel x).
...
Cell 	Vegetation retention 	ALV 	Retained by Cell 
(retained)	Outflow quantity (OQ) from Cell (Gi=1-Ei)
1 	E1 	ALV1 	0 	ALV1 
2 	E2 	ALV2 	ALV1×E2 	ALV1×G2+ALV2 
3 	E3 	ALV3 	((ALV1×G2+ALV2) ×E3	(ALV1×G2+ALV2) ×G3+ALV3 
4 	E4 	ALV4 	ALV1×G2×G3×E4+
ALV2×G3×E4+
ALV3×E4 	ALV1×G2×G3×G4+
ALV2×G3×G4+
ALV3×G4+
ALV4
...
where retainedx is the amount of retention calculated as in the table above, thresh is the total allowed annual load for the pollutant of interest (thresh_p for phosphorous, thresh_n for nitrogen) and contrib is the number of pixels on the landscape. Pixel values are then summed (nret_sm) or averaged (nret_mn) to the sub-watershed scale to give sub-watershed service outputs in biophysical terms.
...
Where:
wp_Valuex is the value of retention for sub-watershed x. 
...
11.  Water Purification Valuation table. This is a table containing valuation information for each of the points of interest. There must be one row for each watershed in the Watersheds layer. 
...
b. c. ann_load: The total critical annual nutrient loading allowed for the nutrient of interest at the point of interest. Floating point value. It has unit of Kg.yr-1.
...
12.  Water Purification threshold table. A table containing annual nutrient load threshold information for each of the points of interest. There must be one row for each watershed in the Watersheds layer. 
...
Columns: Each column contains a different attribute of each watershed and must be named as follows: 
a. ws_id (watershed ID): Unique integer value for each watershed, which must correspond to values in the Watersheds layer.
b. thresh_n / thresh_p: The total critical annual nutrient loading allowed for the nutrient of interest at the point of interest. Floating point value. It has unit of Kg.yr-1.
...
6. Nutrient Loading Coefficients 
...
7. Removal Efficiencies  (eff_n, eff_p)
...
9. Critical Annual Load (Ann_Load) 
...
Parameter	Type	WHO	EU	USEPA
Ammonia	Social	1.5 mg L -1	0.50 mg L -1	No GL
pH 	Social 	6.5-8 	No guidelines 	6.5-8.5 
Chloride 	Social 	250 mg L -1 	250 mg L -1 	250 mg L -1 
Iron 	Social 	0.3 mg L -1 	0.2 mg L -1 	0.3 mg L -1 
Lead 	Health 	0.01 mg L -1 	0.01 mg L -1 	0.015 mg L -1 
Arsenic 	Health 	0.01 mg L -1 	0.01 mg L -1 	0.01 mg L -1 
Copper 	Health 	2.0 mg L -1 	2.0 mg L -1 	1.3 mg L -1 
Fecal Coliform bacteria 	Health 	0 counts/100 mL 	0 counts/100 mL 	0 counts/100 mL 
...
10. Marginal pollutant removal costs (cost)
