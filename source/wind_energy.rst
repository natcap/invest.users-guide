...
The user interface for the model is where all commonly used inputs may be entered. These include information about the type of turbine, number of turbines, the area of interest, etc. To minimize the set of required inputs, the model includes default data on two common offshore wind turbines: 3.6 MW and 5.0 MW. Also included is an input file of default values used to parameterize various parts of the model, called the "Global Wind Energy Parameters" file. These values and the wind turbine values are in CSV files which may be modified if alternate values are desired. When modifying these files, it is recommended that the user make a copy of the default CSV file so as not to lose the original default values.  
...
The wind energy model estimates wind power density (wind power potential) to identify energy rich areas of the ocean. The wind power density :math:`PD (Wm^{-2}`) at a certain location can be approximated as a function of wind statistics (Elliott et al., 1986)
...
The amount of energy harvestable from a wind turbine in a particular location depends on the characteristics of wind turbine as well as wind conditions (Pallabazzer 2003; Jafarian & Ranjbar 2010). The wind energy model quantifies the harvestable energy based on the output power curve of a wind turbine and wind speed statistics. :num:`Figure #power-output-curve-fig` shows an output power curve of a wind turbine (pitch control type). The wind turbine starts to generate power at the cut-in wind speed (Vcin). The output power increases up to the rated power (Prate) as wind speed increases to the rated wind speed (Vrate). The wind turbine keeps producing the maximum power (i.e., Prate) until wind speed reaches the cut-out wind speed (Vcout). If wind speed further increases over the cut-out wind speed, the wind turbine stops to generate power due to safety purposes. Currently, more than 74 offshore wind farms are operating globally and technology specific information of the wind turbine at each wind farm are available at LORC Knowledge (2012).
...
.. figure:: ./wind_energy_images/power_output_curve.png
   ...
   :num:`Figure #power-output-curve-fig` Output power (P) curve of a wind turbine (pitch control type) as a function of wind speed (V) (Modified from Fig.1 in Pallabazzer 2003)

To provide flexibility for a variety of different turbine types without requiring the user to manually enter in a power curve, we estimate the output power P (kW) of a wind turbine using a polynomial modeling approach (Jafarian & Ranjbar 2010): 
...
where, :math:`nday` is the number of days for energy output (e.g. :math:`nday = 365` days for annuatl energy output), :math:`\rho_0` is air density of standard atmosphere (e.g. :math:`1.225 kg m^{-3}` for U.S. standard atmosphere air density at sea level), and :math:`lossrate` is a decimal value which represents energy losses due to a combination of downtime, power conversion efficiency, and electrical grid losses (default value is .05).  All of these parameters are included in the global parameters `.csv` file and may be changed by the user from their defaults. Total farm energy output is equal to the individual turbine output multiplied by the number of turbines, :math:`n`,

.. math:: E = nO
   :label: eq6

The wind energy model also calculates capacity factor CF (%), which is the ratio of the actual output (E) of a power plant over a period of time and its potential output if it would have produced at full capacity (:math:`P_r`)  (Manwell et al. 2009). 


.. math:: \frac{E}{nday\cdot 24hr\cdot P_{rate}} \cdot 100.0
   :label: eq7

Capacity factors vary greatly depending on the wind conditions and turbine characteristics. Typical capacity factor varies around 20-50% (Table 3), and this value can be used to identify an efficient wind turbine for a target area characterized with specific wind conditions.

.. _capacity-factor-fig:

.. figure:: ./wind_energy_images/capacity_factor.png
   :align: center

   :num:`Figure #capacity-factor-fig`. Top 10 offshore wind farms based on installed capacity and technology specific information of the wind turbine installed in each of the farms (Modified from LORC Knowledge 2012).

The InVEST software comes with default technical and financial information about two common turbine sizes, the 3.6 MW and 5.0 MW turbines. The information for each turbine is given in `.csv` files in the `\Input` directory and is a required input into the model. The user can use the default data, edit a file, or create a new file to assess different turbine sizes or update specific characteristics. It is recommended to save edits as new .csv files rather than overwriting the default data.  

Offset Carbon
-------------

To translate carbon-free wind power to a representative amount of annual avoided :math:`\mathrm{CO}_2` emissions, we use the following default conversion factor: :math:`6.8956\cdot 10 ^{-4} \mathrm{metric\ tons\ CO}_2/kWh`
...
The fixed parameter option specifies a mean distance inland along the entire coast that represents the expected distance that overland cables may have to travel to reach a grid connection. Since grid connection points for large farms are very opportunistic and represent a relatively small portion of capital costs, it is not unrealistic to model grid connection this way in the absence of a detailed grid connection scheme. The default parameter included, 5.5 km, is the mean over land cable distance from the UK from the observations.
...
There are a variety of relatively small costs associated with the construction phase, such as those for development, engineering, procurement, and royalties. AWS Truewind (2010) estimate these costs to amount to 2% of total capital expenditures; Blanco (2009) indicates it could be as high as 8%. We adopt their method of calculating these costs and use the mean value of 5% as the default .
...
Since the capital expenditures represent the largest proportion of costs and much of the ancillary costs are fixed fractions of capital costs, it is critically important to validate our model against stated offshore wind farm costs worldwide. To do so, we collected data from http://www.4coffshore.com/ and http://www.lorc.dk/offshore-wind-farms-map/statistics on stated capital costs and designs for wind farms that are in construction or currently operational. We constrained the data collection to only those employing 3.6 MW and 5.0 MW turbines, for which we have provided default data with the InVEST model. Stated capital costs from 4Coffshore were inflated to 2012 $US using the financial close information as the basis for when the cost estimate was collected. To generate predictions, the design of each farm was input into the InVEST model using appropriate default cost parameters for all components. Most farms have their own electrical transmission equipment, though some deepwater farms are beginning to used centralized offshore substations that aggregate energy for transport from multiple farms. To predict electrical transmission costs for these farms, it was first necessary to estimate the cost of the entire offshore substation and then attribute a prorated capital cost to each farm based on their relative contribution to exported energy capacity. For example, an offshore substation with a 800 MW export capacity that is connected to Farm A (200 MW) and Farm B (600 MW) would contribute 25% of capital costs to Farm A and 75% to Farm B. 
...
.. figure:: ./wind_energy_images/project_costs.png
   ...
   Plots the predicted capital costs against stated capital costs. The results show a very strong correlation between predictions and stated capital costs for 3.6 MW and 5.0 MW turbines using the default data. 
...
The quality of wind input data determines the accuracy of model results. So, users need to understand the quality of wind input data for proper interpretation of the model results. For example, the default wind input data are more appropriate for global and regional scale applications at 4 or 60 minutes spatial resolution. For a more detailed analysis of wind power and energy in a target area, the user may want to provide wind data observed (or modeled) at a finer spatial resolution.
...
2. **Wind Data Points (required).**  A binary file that represents the wind input data (Weibull parameters). This parameter box should point to one of two files provided by our model. These files are found in the `WindEnergy\input` direction inside the InVEST installation directory.
  ...
  * North East Coast of the US: `ECNA_EEZ_WEBPAR_Aug27_2012.bin` for finer resolution of that area.
...
13. **Cost of the Foundation Type (millions of dollars)** A floating point number for the unit cost of the foundation type (in millions of dollars). The cost of a foundation will depend on the type selected, which itself depends on a variety of factors including depth and turbine choice.
...
16. **Discount Rate** The discount rate reflects preferences for immediate benefits over future benefits.  (Ex: 1% as 0.01, 100% as 1.0).
 ...
 * `example_size_and_orientation_of_a_possible_wind_farm.shp`: an ESRI Shapefile that represents polyline centered at a randomly selected wind data point to indicate the perimeter of  the farm.
 ...
 * `npv_US_millions.tif`: a GeoTIFF raster file that represents the net present value of a farm placed at the center of the pixel.

 * `wind_energy_points.shp`: an ESRI Shapefile that summarizes the above outputs in each point...
 ...
 * Water depth: NOAA’s National Geophysical Data Center (NGDC) provides global bathymetry data with various spatial resolutions at http://www.ngdc.noaa.gov/mgg/bathymetry/relief.html.
  ...
  * Parameters for output power curve of a wind turbine: cut-in wind speed (:math:`V_{cin}`), cut-out wind speed (:math:`V_{cout}`), rated wind speed (:math:`V_{rate}`),  and rated power (:math:`P_{rate}`). 
